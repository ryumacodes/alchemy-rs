...
/// SSE stream processor for APIs that don't send `[DONE]` sentinel.
///
/// Google Generative AI uses `alt=sse` which produces standard SSE lines
/// but ends via connection close rather than a `[DONE]` message.
